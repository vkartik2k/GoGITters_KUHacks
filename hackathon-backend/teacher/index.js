                                            ...
                                            if (variables.sessionId === '') {
                                                ...
                                                for (let i = 0; i < files.length; i++) {
                                                    ...
                                                    fetch("https://api.cloudinary.com/v1_1/demo/image/upload", {
                                                        ...
                                                        .then((data) => {
                                                            // variables.uploadFile=  JSON.parse(data).url.split("upload").join("upload/fl_attachment:temp")
                                                            variables.uploadFile = JSON.parse(data).url

                                                            console.log(variables.uploadFile)
                                                            ...
                                                            postData('../../user/' + user._id + '/classroom/' + element._id + '/createSession', inputJson)
                                                                .then(data => {
                                                                    ...
                                                                    variables.sessionId = data.session._id
                                                                    $('.classContent').append(templates.classContent3)
                                                                    setTimeout(function () {
                                                                        $('#startRound').click(function () {
                                                                            if (variables.round === '') {
                                                                                let pg = $('#pageNo').val()
                                                                                postData('../../user/' + user._id + '/classroom/' + element._id + '/session/' + variables.sessionId + '/createRound', {
                                                                                    pageNumber: pg
                                                                                })
                                                                                    .then(data => {
                                                                                        $('#startRound').html('Stop Round')
                                                                                        $("#pageNo").hide()
                                                                                        variables.round = data.round._id
                                                                                    })
                                                                            }
                                                                            else {
                                                                                postData('../../user/' + user._id + '/classroom/' + element._id + '/session/' + variables.sessionId + '/endRound/' + variables.round, {})
                                                                                    .then(data => {
                                                ...
                                                postData('../../user/' + user._id + '/classroom/' + element._id + '/endSession/' + variables.sessionId, {})
                                                    .then(data => {
                                                        $('#sessionName').show()
                                                        $('#cutoff').show()
                                                        $("#uploadPpt").show()
